#!/usr/bin/env python3
"""
Migrate providers to plugin structure.
...
DOMAINS = {
    "embeddings": [
        "openai",  # Already migrated
        "openrouter",  # Already migrated
        "local",  # Already migrated
        "numpy_provider",
        "hash",
        "cohere",
        "fastai",
...
def create_plugin_structure(domain, provider):
    ...
    # Special case for numpy_provider which has a different filename
    provider_file = provider
    if provider == "numpy_provider":
        provider = "numpy"
    ...
    src_file = Path(f"pepperpy/{domain}/providers/{provider_file}.py")
    if src_file.exists():
...
if __name__ == "__main__":
    ...
    print("\nDone! All providers migrated to plugins.")
