#!/usr/bin/env python3
"""
Migrate additional providers to plugin structure.

This script copies provider implementations from the main library
...
def create_plugin_structure(domain, provider, src_path=None):
    """Create plugin structure for a provider."""
    # Handle special domains that have subdirectories
    real_domain = domain
    ...
    # Special handling for tools repository
    if domain == "tools_repository":
        real_domain = "tools/repository"
        plugin_domain = "tools_repository"
        src_path_override = f"pepperpy/{real_domain}/providers/{provider}.py"

    # Create plugin directory
    plugin_dir = Path(f"plugins/{plugin_domain}_{provider}")
    ...
    if not plugin_yaml.exists():
        ...
        domain_title = plugin_domain.replace("_", " ").title()
        plugin_description = f"{provider_title} for {plugin_domain} tasks"
        ...
        with open(plugin_yaml, "w") as f:
            f.write(
                PLUGIN_YAML_TEMPLATE.format(
                    ...
                    domain=plugin_domain.replace("_", "/"),  # For nested domains
                    provider=provider,
                )
            )
    ...
    src_file = Path(
        ...
        else f"pepperpy/{real_domain}/providers/{provider}.py"
    )
    if src_file.exists():
        ...
        backup_file = backup_dir / f"{plugin_domain}_{provider}.py.bak"
...
if __name__ == "__main__":
    ...
    print("\nDone! All additional providers migrated to plugins.")
