...
from pepperpy.embedding import CachedEmbedder, TextEmbedder
...
async def main():
    ...
    print(
        f"Generated {len(embeddings)} embeddings in {end_time - start_time:.4f} seconds"
    )
    ...
    end_time = asyncio.get_event_loop().time()
    print(
        f"Retrieved {len(embeddings)} embeddings in {end_time - start_time:.4f} seconds"
    )
    ...
    end_time = asyncio.get_event_loop().time()
    print(
        f"Retrieved/generated {len(embeddings)} embeddings in {end_time - start_time:.4f} seconds"
    )
