...
class VQLiteProvider(RAGProvider):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize VQLite provider.

        Args:
            persist_directory: Directory to persist the database. If None, uses ~/.pepperpy/vqlite
        ...
        self.metric = metric
        self.kwargs = kwargs
        self._db: Optional[VQLite] = None

    def _get_db(self) -> VQLite:
    ...
    async def initialize(self) -> None:
        ...
        db = VQLite(
            dim=self.dimension,
            metric=self.metric,
            path=os.path.join(self.persist_directory, "vectors.vql"),
            **self.kwargs,
        )

        # Load existing data if any
        if os.path.exists(os.path.join(self.persist_directory, "vectors.vql")):
            db.load()
