"""OasysDB provider implementation for RAG capabilities.

This module provides an OasysDB-based implementation of the RAG provider interface,
using OasysDB for high-performance vector storage and retrieval with Rust backend.
...
@lazy_provider_class("rag", "oasys")
class OasysProvider(BaseProvider):
    """OasysDB implementation of the RAG provider interface.

    This provider supports:
    - High performance vector operations with Rust backend
    - Memory safety and efficient resource usage
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize OasysDB provider.

        Args:
            path: Path to store vector data
            collection_name: Name of the collection to use
            **kwargs: Additional configuration options
        """
        super().__init__(**kwargs)

        self.path = path or "oasys_data"
    ...
    async def initialize(self) -> None:
        ...
        This method sets up the OasysDB database and collection.
        """
        # Import oasysdb only when provider is instantiated
        oasysdb = import_provider("oasysdb", "rag", "oasys")

        # Initialize database
        os.makedirs(self.path, exist_ok=True)
        self.db = oasysdb.Database(self.path)
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_batch_size": 10000,  # Higher due to Rust backend
            "max_dimensions": 1536,
        }
    ...
    async def add_documents(
        ...
        **kwargs: Any,
        ...
        # Convert documents to OasysDB format
