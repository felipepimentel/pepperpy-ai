    ...
    async def initialize(self) -> None:
        ...
        async with self._state_lock:
            if self._state != LifecycleState.CREATED:
                raise StateError(
                    f"Cannot initialize component in {self._state} state"
                )
            ...
            try:
                ...
                self._logger.error(f"Failed to initialize component: {e}", exc_info=True)
                raise
    ...
    async def stop(self) -> None:
        ...
        async with self._state_lock:
            if self._state != LifecycleState.RUNNING:
                raise StateError(
                    f"Cannot stop component in {self._state} state"
                )
