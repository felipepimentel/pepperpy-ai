...
class AccuracyMetric(BaseMetric):
    ...
    def compute(self, data: Dict[str, Any]) -> MetricResult:
        ...
        return MetricResult(type=MetricType.ACCURACY, value=accuracy)
...
class PrecisionRecallMetric(BaseMetric):
    ...
    def compute(self, data: Dict[str, Any]) -> MetricResult:
        ...
        false_positives = sum(
            1 for p, g in zip(predictions, ground_truth) if p and not g
        )
        false_negatives = sum(
            1 for p, g in zip(predictions, ground_truth) if not p and g
        )
        ...
        return MetricResult(type=metric_type, value=value)
...
class CostEfficiencyMetric(BaseMetric):
    ...
    def compute(self, data: Dict[str, Any]) -> MetricResult:
        ...
        if not successful_costs:
            return MetricResult(type=MetricType.COST_EFFICIENCY, value=0.0)
