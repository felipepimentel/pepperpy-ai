...
class TaskCompletionBenchmark(BaseBenchmark):
    ...
    def _check_constraints(self, output: Any, constraints: Dict[str, Any]) -> List[float]:
        """Check constraint violations and return penalties."""
        penalties = []
        
        if 'max_time' in constraints and constraints['max_time'] < output.get('time', 0):
    ...
    def get_summary(self, results: Dict[str, List[BenchmarkResult]]) -> Dict[str, Any]:
        ...
        for benchmark_name, benchmark_results in results.items():
            ...
            summary['benchmarks'][benchmark_name] = {
                ...
                'average_duration': sum(r.duration_ms for r in benchmark_results) / len(benchmark_results)
            }
        ...
        summary['overall']['average_score'] = total_score / total_cases if total_cases > 0 else 0.0
        summary['overall']['success_rate'] = (
            summary['overall']['successful'] / summary['overall']['total_cases']
            if summary['overall']['total_cases'] > 0 else 0.0
        )
