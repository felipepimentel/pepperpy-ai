...
class AudioPipeline:
    ...
    @staticmethod
    def create_segment(
        title: str, content: str, speaker: str = "default", duration: str = ""
    ) -> AudioSegment:
        ...
        """
        return AudioSegment(
            title=title, content=content, speaker=speaker, duration=duration
        )
    ...
    async def load_content(
        ...
        try:
            ...
            if isinstance(source, dict):
                ...
                self.log(
                    VerbosityLevel.INFO,
                    f"Loaded project '{result.title}' from dictionary",
                )
                return result
            ...
            if content_type in ("json", ""):
                with open(path) as f:
                    ...
                    self.log(
                        VerbosityLevel.INFO,
                        f"Loaded project '{result.title}' from {path}",
                    )
                    return result
            else:
    ...
    async def generate_audio(
        ...
        try:
            ...
            self.log(
                VerbosityLevel.INFO,
                f"Generating audio for {len(project.segments)} segments",
            )
            ...
            for i, segment in enumerate(project.segments):
                ...
                if not segment.content.strip():
                    self.log(
                        VerbosityLevel.DEBUG, f"Skipping empty segment: {segment.title}"
                    )
                    continue

                # Determine voice ID from speaker mapping
                voice_id = self.voice_mapping.get(
                    segment.speaker, self.voice_mapping["default"]
                )

                self.log(
                    VerbosityLevel.DEBUG,
                    f"Processing segment {i + 1}/{len(project.segments)}: '{segment.title}' with voice {voice_id}",
                )

                # Generate audio
                audio = await convert_text(
                    text=segment.content, voice_id=voice_id, provider=provider, **kwargs
                )
                ...
                self.log(
                    VerbosityLevel.INFO, f"Generated audio for segment: {segment.title}"
                )

            self.log(VerbosityLevel.INFO, "Completed audio generation for all segments")
            return audio_files
    ...
    async def add_sound_effects(
        ...
        **kwargs,
        ...
        try:
            ...
            self.log(
                VerbosityLevel.INFO,
                f"Adding sound effects to {len(audio_files)} audio files",
            )

            if background_music:
                self.log(
                    VerbosityLevel.INFO, f"Using background music: {background_music}"
                )
            ...
            for title, file_path in audio_files.items():
                ...
                if os.path.exists(file_path):
                    ...
                    self.log(
                        VerbosityLevel.DEBUG,
                        f"Enhanced audio saved to: {enhanced_path}",
                    )
                else:
                    # Simulated output for development/example purposes
                    enhanced_files[title] = enhanced_path
                    self.log(
                        VerbosityLevel.DEBUG, f"Simulated enhanced audio for: {title}"
                    )

            self.log(
                VerbosityLevel.INFO, "Completed adding sound effects to all audio files"
            )
    ...
    async def compile_audio(
        ...
        **kwargs,
        ...
        try:
            ...
            self.log(
                VerbosityLevel.INFO,
                f"Successfully compiled final audio to: {final_path}",
            )
            return final_path
    ...
    async def process_project(
        self,
        source: Union[str, Dict[str, Any], AudioProject],
        output_path: Optional[str] = None,
        background_music: Optional[str] = None,
        **kwargs: Any,
    ) -> str:
        """Process an audio project from start to finish.
        ...
        try:
            ...
            project = (
                source
                if isinstance(source, AudioProject)
                else await self.load_content(source)
            )
            ...
            if output_path:
                ...
                self._ensure_directory(self.output_dir)
                self.log(
                    VerbosityLevel.DEBUG, f"Set output directory to: {self.output_dir}"
                )
            ...
            enhanced_files = await self.add_sound_effects(
                audio_files, background_music=background_music, **kwargs
            )
            ...
            self.log(VerbosityLevel.INFO, "Project processing completed successfully")
            return final_path
