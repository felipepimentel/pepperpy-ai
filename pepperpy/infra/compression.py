...
Typical usage:
    from pepperpy.infra.compression import compress_data, decompress_data
        ...
        try:
            if self.serialization == SerializationFormat.JSON:
                ...
                if isinstance(data, bytes):
                    ...
                    raise ValueError(
                        f"Cannot serialize {type(data).__name__} as raw bytes"
                    )
            else:
                raise ValueError(
...
class NumpyCompressor:
    ...
    def decompress(self, data: bytes) -> np.ndarray:
        ...
        Args:
            data: The compressed array data
        ...
        Raises:
            ValueError: If the data cannot be decompressed
        """
        # Create a compressor with the same settings
        compressor = DataCompressor(
            algorithm=self.algorithm,
            serialization=SerializationFormat.RAW,
            level=self.level,
        ...
        if not isinstance(array, np.ndarray):
            raise ValueError(f"Expected ndarray, got {type(array).__name__}")
...
class JsonCompressor:
    ...
    def decompress(self, data: bytes) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        ...
        json_data = combined_data.get("data")
        key_map = combined_data.get("key_map")

        # Restore the original keys if optimized
        if key_map is not None:
            # Ensure json_data is of the correct type before passing to _restore_json
            ...
            if isinstance(json_data, dict):
                ...
                raise ValueError(f"Unexpected data type: {type(json_data)}")

    def _optimize_json(
        self, data: Union[Dict[str, Any], List[Dict[str, Any]]]
    ) -> Tuple[Union[Dict[int, Any], List[Dict[int, Any]]], Dict[int, str]]:
        """Optimize JSON data by replacing string keys with integer IDs.
        ...
        Returns:
            ...
            keys and key_map maps integer IDs to original keys
        """
        ...
        if isinstance(data, dict):
            optimized_data: Union[Dict[int, Any], List[Dict[int, Any]]] = {
                reverse_map[key]: value for key, value in data.items()
            }
        else:
            optimized_data_list: List[Dict[int, Any]] = []
            for item in data:
                if isinstance(item, dict):
                    optimized_item = {
                        reverse_map[key]: value for key, value in item.items()
                    }
                    optimized_data_list.append(optimized_item)
                else:
                    # Handle non-dict items if they exist in the list
                    optimized_data_list.append(item)  # type: ignore
            optimized_data = optimized_data_list
    ...
    def _restore_json(
        ...
        # Helper function to convert keys in a dictionary
        def convert_dict_keys(d: Dict[Any, Any]) -> Dict[str, Any]:
            result: Dict[str, Any] = {}
            for k, v in d.items():
                # Convert the key
                if isinstance(k, int) and k in key_map:
                    new_key = key_map[k]
                elif isinstance(k, str) and k.isdigit() and int(k) in key_map:
                    new_key = key_map[int(k)]
                else:
                    new_key = str(k)

                # Convert the value recursively if it's a dict or list
                if isinstance(v, dict):
                    result[new_key] = convert_dict_keys(v)
                elif isinstance(v, list):
                    result[new_key] = convert_list_items(v)
                else:
                    result[new_key] = v
            return result

        # Helper function to convert items in a list
        def convert_list_items(lst: List[Any]) -> List[Any]:
            result: List[Any] = []
            for item in lst:
                if isinstance(item, dict):
                    result.append(convert_dict_keys(item))
                elif isinstance(item, list):
                    result.append(convert_list_items(item))
                else:
                    result.append(item)
            return result
        ...
        if isinstance(data, dict):
            ...
            return convert_list_items(data)
        else:
            # This should not happen given the type annotation, but just in case
